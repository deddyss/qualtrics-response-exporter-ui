<template>
	<Listbox as="div" v-model="selected" :disabled="disabled">
		<div class="mt-1 relative">
			<ListboxButton class="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm cursor-pointer disabled:opacity-50 disabled:cursor-default">
...
export default defineComponent({
	...
	setup(props) {
		...
		const label = computed(() => {
			...
			// props.options.forEach((item: SelectOption) => {
			// 	console.log(item);
			// });
